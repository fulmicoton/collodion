...
public class NumberParserFilter extends TokenFilter {
    ...
    public static class Builder implements ProcessorBuilder<NumberParserFilter> {
        ...
        @Override
        public NumberParserFilter createFilter(final TokenStream prev) throws IOException {
    ...
    public boolean peek(final int pos) throws IOException {
        // stateQueue = 0, posAhead = 0 --> 1
        while (this.stateQueue.end() <= pos) {
            if (!this.input.incrementToken()) {
                return false;
            }
            this.stateQueue.push();
        }
        this.stateQueue.loadState(pos);
        return true;
    }
    ...
    public final boolean incrementToken() throws IOException {
        ...
        for (int numTokens = 0; numTokens < MAX_STATES; numTokens++) {
            if (!this.peek(this.tokenOffset + numTokens)) {
                if (numTokens == 0) {
            ...
            for (int charId = 0; charId < this.termAttr.length(); charId++) {
                final char c = this.termAttr.charAt(charId);
        ...
        if (matchedPattern == -1) {
            ...
            final double val = numberInterpreter.read(buffer.toString());
